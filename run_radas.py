...
def run_radas(case: str, show: bool):
    ...
    if case == "all":
        cases = read_cases.list_cases()
        print(f"Running all cases: {cases}")
        with mp.Pool() as pool:
            pool.map(run_radas_for_case, cases)
    else:
...
def run_radas_for_case(case: str):
    ...
    dataset = dataset.assign_attrs(radas_git_hash = get_git_revision_short_hash())

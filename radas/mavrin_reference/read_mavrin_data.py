...
def compute_Mavrin_polynomial_fit_single(Te_eV, ne_tau_s_per_m3, coeff, warn: bool=False):
    ...
    if not Tmin_eV[0] <= Te_eV <= Tmax_eV[-1]:
        if warn: warnings.warn(f"{Te_eV}eV outside fitted range {Tmin_eV[0]}eV to {Tmax_eV[-1]}eV")
        return np.nan
    if ne_tau_s_per_m3 < 1e15:
        if warn: warnings.warn(f"{ne_tau_s_per_m3} outside fitted range above 1e16 m^-3 s")
        return np.nan

    X = np.log10(Te_eV)
    Y = np.log10(ne_tau_s_per_m3 / 1e19)
    if warn and (Y > 0.0): warnings.warn(f"Warning: treating points with ne_tau_s_per_m3 > 1e19 m^-3 s as coronal.")
    Y = np.minimum(Y, 0.0)
